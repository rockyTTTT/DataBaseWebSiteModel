  ...
  methods: {
    ...
    handleCommand(command) {
      ...
      if (this.selected.length != 0) {
        ...
        let input = document.getElementsByClassName("select-options");
        console.log('input',input);
        // 取上一个tag
        let lastTag = searchTags[searchTags.length - 1];
        if (lastTag != null) {
          console.log("rect", lastTag.getBoundingClientRect());
          // console.log("rect_input", input.getBoundingClientRect());
        }
      } else {
