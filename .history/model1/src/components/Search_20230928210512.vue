...
export default {
  ...
  methods: {
    ...
    handleClose(index) {
      ...
      if (this.selected.length === 0) {
        ...
        if(lastTag != null){
          ...
          console.log('lastTagBound',lastTagBound);
          console.log('inputBound',inputBound);
          // 只要上一个标签的起始位置在第一个，并且不在第一排，则减少height
          if(lastTagBound.x < inputBound.x + 15  && lastTagBound.y > inputBound.y + 20){
            this.inputHeight -= 40;
    ...
    handleCommand(command) {
      ...
      if (this.selected.length != 0) {
        console.log('现在执行handleCommand方法');
        ...
        if (lastTag != null) {
          let lastTagBound = lastTag.getBoundingClientRect();
          let inputBound = input.getBoundingClientRect();
          if(lastTagBound.x + lastTagBound.width  > inputBound.x + inputBound.width - 100){
            this.inputHeight += 40;
            const inputInner = inputs[0].querySelector('.el-input__inner');
            if(inputInner){
              inputInner.style.height = this.inputHeight + 'px';
