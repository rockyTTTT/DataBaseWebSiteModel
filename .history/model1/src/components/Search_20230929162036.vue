...
export default {
  ...
  data() {
    return {
      ...
      showCancer:true,
      list:[
        {displayValue:"false",label:"Cancer",value:"Anal canal cancer"},
        {displayValue:"false", value: "Bladder cancer", label: "Bladder cancer" },
        {displayValue:"false", value: "Bile duct cancer", label: "Bile duct cancer" },
        {displayValue:"false",value: "Breast cancer", label: "Breast cancer" },
        {displayValue:"false", value: "Colon cancer", label: "Colon cancer" },
        {displayValue:"false", value: "Gastric cancer", label: "Gastric cancer" },
        {displayValue:"false", value: "Kidney cancer", label: "Kidney cancer" },
      ] ,
      selected: [],
      inputHeight: 40,
    };
  ...
  methods: {
    ...
    handleClose(index) {
      ...
      if (this.selected.length === 0) {
        ...
        if (lastTag != null) {
          ...
          // 只要上一个标签的起始位置在第一个，并且不在第一排，则减少height
          if (
            lastTagBound.x < inputBound.x + 15 &&
            lastTagBound.y > inputBound.y + 20
          ) {
            this.inputHeight -= 40;
            const inputInner = inputs[0].querySelector(".el-input__inner");
            if (inputInner) {
              inputInner.style.height = this.inputHeight + "px";
            }
          }
        }
      }
    ...
    handleCommand(command) {
      ...
      if (this.selected.includes(command)) {
        ...
        this.handleClose(this.selected.indexOf(command));
        return;
      } else {
      ...
      if (this.selected.length != 0) {
        ...
        if (lastTag != null) {
          ...
          let inputBound = input.getBoundingClientRect();
          if (
            lastTagBound.x + lastTagBound.width >
            inputBound.x + inputBound.width - 100
          ) {
            this.inputHeight += 40;
            const inputInner = inputs[0].querySelector(".el-input__inner");
            if (inputInner) {
