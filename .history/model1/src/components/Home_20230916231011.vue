  ...
  methods: {
    updateCircles(origin, destination, direction) {
      ...
      circles.forEach((circle, index) => {
        ...
        circle.style.width = index === activeSectionIndex ? "16.8rem" : "9.8rem";
        circle.style.height = index === activeSectionIndex ? "16.8rem" : "9.8rem";
      });
    },
...
.fixed-nav {
  ...
  box-shadow: 0rem 2.8rem 5.6rem rgba(0, 0, 0, 0.2);
  ul {
    padding: 0;
    margin: 0;
  }
...
.router-active {
  .nav-names {
    border-bottom: 3rem solid #f6903d;
  }
}
.title {
  font-size: 126rem;
...
.custom-dropdown-item {
  font-size: 24rem; /* 修改文本大小 */
  padding: 12rem 2px; /* 修改框的大小，上下8px，左右20px */
}
